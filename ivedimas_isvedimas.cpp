...
void isvedimas_i_faila(std::vector<Studentas> S){
    ...
    // }
    std::ofstream galv("galvociai.txt");
    std::ofstream dund("dundukai.txt");
    if(!galv || !dund){
        std::cerr << "Klaida! Nepavyko sukurti failo...\n";
        return;
    }
    galv << std::left << std::setw(15) << "Vardas" << std::left << std::setw(25) << "Pavarde" << std::left << std::setw(18) << "Galutinis (Vid.)" << std::left << std::setw(18) << "Galutinis (Med.)\n";
    for (int i = 0; i < 76; i++) {
        galv << "-";
    }
    galv << std::endl;
    dund << std::left << std::setw(15) << "Vardas" << std::left << std::setw(25) << "Pavarde" << std::left << std::setw(18) << "Galutinis (Vid.)" << std::left << std::setw(18) << "Galutinis (Med.)\n";
    for (int i = 0; i < 76; i++) {
        dund << "-";
    }
    dund << std::endl;
    for (auto& s : S) {
        if(s.grupe == galvociai){
            galv << std::left << std::setw(15) << s.Vardas << std::left << std::setw(25) << s.Pavarde << std::left << std::setw(18) << std::fixed << std::setprecision(2) << s.galutinis_vid << std::left << std::setw(18) << std::fixed << std::setprecision(2) << s.galutinis_med << std::endl;
        }
        else{
            dund << std::left << std::setw(15) << s.Vardas << std::left << std::setw(25) << s.Pavarde << std::left << std::setw(18) << std::fixed << std::setprecision(2) << s.galutinis_vid << std::left << std::setw(18) << std::fixed << std::setprecision(2) << s.galutinis_med << std::endl;
        }
    }
}
